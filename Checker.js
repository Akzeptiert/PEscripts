...
const showEnchants = new StateSetting("Зачарование", false);
...
durabilityModule.setOnClickListener(function(view) {
    ...
    if (maxDurability > 0) {
        message = "§aПрочность: " + currentDurability + "/" + maxDurability + "\n" +
                  "§7ID: " + fullId;
        if (showEnchants.isActive()) {
            message += "\n" + (Item.isEnchanted(selectedSlot) ? "§bЗачарован" : "§eНе зачарован");
        }
    } else {
        message = "§cПрочность: Отсутствует" + "\n" +
                  "§7ID: " + fullId;
        if (showEnchants.isActive()) {
            message += "\n" + (Item.isEnchanted(selectedSlot) ? "§bЗачарован" : "§eНе зачарован");
        }
